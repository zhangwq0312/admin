Ext.define('app.view.module.advert.Controller', {
	...
	onAddBtn: function() {
    	var win = this.lookupReference('advert_window');
    	
    	if (!win) {
            win = Ext.create('app.view.module.advert.AdvertWindow', {
   ...
   submitCommon: function(win) {
	...
	onPay: function(view,rowIndex,colIndex,item,e,record) {
		...
		s.filterBy(function(record){  
			return record.raw.status > -1&&record.raw.type=='advert_valid';  
		});  
		s.reload();
	
	   	var win = this.lookupReference('advert_paywindow');
    	
    	if (!win) {
            win = Ext.create('app.view.module.advert.PayWindow', {
            	viewModel: this.getView().getViewModel()
            });
            this.getView().add(win);
	...
	change: function(view,rowIndex,colIndex,item,e,record,type,title) {
		...
		Ext.Msg.show({
		    title:title,
		    message: '名称：' + record.raw.advert_name,
	...
	core: function(view,rowIndex,colIndex,item,e,record,type) {
    	...
    	Ext.Ajax.request({
    		url: 'advert/change.do',
    		async: true,
    		params: {
				type:type,
    			advert_id: record.raw.advert_id,
    		},
    		scope: this,
	...
	parseAdvertTypeV: function(v) {
    	var str = '';
    	if(this.mainViewModel != null && this.mainViewModel.get('advertTypeStore') != null) {
    		str = this.mainViewModel.parseValue(v, 'advertTypeStore', 's_id', 's_name');
    	}
    	return str;
    },
